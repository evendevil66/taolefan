...
CREATE TABLE `orders` (
  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT '自增id 自增主键',
  ...
  `tk_create_time` datetime DEFAULT NULL COMMENT '下单时间',
  `tk_earning_time` datetime DEFAULT NULL COMMENT '结算时间',
  `tk_status` int(11) NOT NULL COMMENT '订单状态 3：订单结算，12：订单付款， 13：订单失效，14：订单成功',
  `pay_price` decimal(32,10) DEFAULT NULL COMMENT '付款金额',
  `pub_share_pre_fee` decimal(32,10) DEFAULT NULL COMMENT '付款预估收入',
  `pub_share_fee` decimal(32,10) DEFAULT NULL COMMENT '结算预估收入',
  `tk_commission_pre_fee_for_media_platform` decimal(32,10) DEFAULT NULL COMMENT '预估内容专项服务费',
  `tk_commission_fee_for_media_platform` decimal(32,10) DEFAULT NULL COMMENT '结算内容专项服务费',
  `share_pre_fee` decimal(32,10) DEFAULT NULL COMMENT '预估专项服务费',
  `share_fee` decimal(32,10) DEFAULT NULL COMMENT '结算专项服务费',
  `rebate_pre_fee` decimal(32,10) DEFAULT NULL COMMENT '预估返利金额',
  `rebate_fee` decimal(32,10) DEFAULT NULL COMMENT '结算返利金额',
  `refund_tag` varchar(1) DEFAULT NULL COMMENT '维权状态',
  `rebate_status` varchar(1) NOT NULL COMMENT '结算状态',
  PRIMARY KEY (`id`)
...
CREATE TABLE `percentage` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='提成统计表 ';
...
CREATE TABLE `users` (
  ...
  `channel_id` varchar(32) DEFAULT NULL COMMENT '渠道id 暂时不使用',
